...
transition point between sections.
...
\ENSURE{Segment boundaries $B^m$, frame labels $C^m$ for each $m \in 2\dots M$}\\
...
spans a much wider range of genres and instrumentation, and provides multiple
annotation levels for each track, ranging from small-scale segments to large
functional components as in the Beatles.
\end{description}

In each evaluation, we report the $F$-measure of boundary detection at 0.5-second and
3-second windows, as well as pair-wise $F$-measure for frame
classification~\cite{levy2008structural}.  For comparison purposes, we also report
scores achieved by the method of Serr\`{a} \etal~\cite{serra2014unsupervised}.

\subsection{Implementation details}
All input signals are sampled at 22050Hz, and analyzed with a 2048-sample FFT window
and 512-sample hop.  Repetition similarity matrices $S^\text{rep}$ were 
computed by first extracting log-power constant-Q transforms from $C2$ (32.7Hz) to
$C8$ (2093.0Hz), using a down-sampling method 
similar to that of Sch{\"or}rkhuber and Klapuri~\cite{schorkhuber2010constant}.  CQT
frames are mean-aggregated between detected beat events, and stacked using time-delay
embedding with one step of history.
Similarity matrices were the computed by applying a Gaussian kernel
$S^\text{rep}_{ij} \defeq \exp\left(-\frac{1}{2\sigma^2}\|x_i - x_j\|^2\right)$ to 
each pair of beat-synchronous frames $i$ and $j$.
The bandwidth was estimated by computing the average squared distance between each
$x_i$ and its $k$th nearest neighbor, and $k$ was set to $1 + \lceil 2\log_2 n\rceil$.
...
\footnote{All methods were implemented in Python, and source code will be made freely available
upon publication.}
