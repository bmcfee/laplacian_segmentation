...
samples of a particular sequence will may not be connected.
...
local sequential connections and global repetition structure, but it does not balance the 
...
In this regime, subsequent analysis of the graph may fail to detect sequential structure.
...
$\mu$, this indicates that the combination should be weighted such that the local
...
method for finding normalized graph cuts~\cite{von2007tutorial}, and it is well-suited to detecting
...
specified maximum number of structural components types.  Varying $M$ --- equivalently, 
...
\STATE{$R \leftarrow $ \cref{unfiltered-rep} on $S^\text{rep}$} \COMMENT{Recurrence
detection}
