...
\twoauthors%
 ...
 {Daniel P.W. Elllis} {LabROSA \\ Columbia University \\ {\tt dpwe@ee.columbia.edu }}
...
itself has more general application in the simultaneous analysis of structure at multiple 
...
differs by using local timbre consistency to connect sequences of long-range tonal repetitions.
...
To facilitate discovery of repeated sections, we modify $R'$ by 
...
belonging to the same component, but this information is of course hidden.  By adding 
links along the first diagonals, the graph becomes fully connected, but 
repeated sections will exhibit additional internal connectivity structure from the recurrence links.  
Let $i$ and $j$ denote two repetitions of the same sample at different times; then $R^+$ should
...
The construction of \cref{pathmatrix} describes the intuition behind the combination of
local sequential connections and global repetition structure, but it does not explicitly
...
If we allow weights on the edges, then the combination can be parameterized 
...
The construction above relies upon a single feature representation $X$ to 
...
optimized by the~\eqref{optweight:solution} on the weighted affinity matrices.
...
\caption{Pair-wise frame similarity ($Y\trans Y$) using the first $10$ components for \emph{The Beatles --- Come Together}.  The first
...
specified maximum number of structural components types.  By varying $M$ --- equivalently, 
the dimension of the representation --- we can directly adjust the granularity of 
...
{\sc Boundary-Detect}$(Y)$
    ...
    \STATE{Let $c_t$ denote the cluster containing $\hat{y}_t$}
...
All methods were implemented in Python with librosa~\cite{librosa}.
...
Consequently, it may be beneficial in practice to provide segmentations at multiple resolutions.
